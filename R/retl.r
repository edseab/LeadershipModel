retl <- function(px,pavg,grpsz=5, L=3,LL=0.8, Lavg = 3, P=1, Pavg=1, banned=0,inv = 0.6,volcost=0.1, Lcost=0.2, E=0.5, Eavg=0.5, Ecoef = 0.9, Baseline=5){
...
L_return_ego <- ((LL + (L-LL)*inv)*grpsz - E)/grpsz + Baseline # Leadership of ego
L_return_avg <- ((LL + (Lavg-LL)*inv)*grpsz - Eavg)/grpsz  + Baseline# Expected group Leadership

Ex <- ((1-px)*p_no_vol*(LL+P + Baseline) + # Return if no leader
