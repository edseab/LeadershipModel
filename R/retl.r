retl <- function(px,pavg,grpsz=5, L=3,LL=0.8, Lavg = 1.5, P=1, Pavg=1, inv = 0.6,volcost=0.1, Lcost=0.2, E=0.5, Eavg=0.5, Ecoef = 0.9){


p_no_vol <- (1-pavg)^(grpsz-1) # prob that no one else volunteers

p_elected_vol <- sum(dbinom(0:(grpsz-1),(grpsz-1),pavg)*(1/(1:grpsz)))
...
Ex <- (1-px)*p_no_vol*(LL+P) + # Return if no leader
      (1-px)*(1-p_no_vol)*(L_return_avg + P - Eavg) + # Return if ego doesnt volunteer but someone else does
